...
class SparseSegmentReductionHelper(SegmentReductionHelper):
  ...
  def _sparseSegmentReduceGrad(self, ygrad, indices, segment_ids, output_dim0,
                               is_sqrtn):
    weights = np.zeros_like(ygrad, shape=ygrad.shape[0])
    for segment in segment_ids:
      weights[segment] += 1
    weights = 1. / weights if not is_sqrtn else 1. / np.sqrt(weights)
    xgrad = np.zeros_like(ygrad, shape=[output_dim0, ygrad.shape[1]])
  ...
  def testGradientExplicit(self):
    ...
    for inner_size in (1, 2, 3, 32):
      with self.session():
        ...
        output_dim0 = 10
        ops_list = [
            (math_ops.sparse_segment_mean_grad, False),
            (math_ops.sparse_segment_sqrt_n_grad, True),
        ]
        for tf_op, is_sqrtn in ops_list:
          np_xgrad = self._sparseSegmentReduceGrad(
              np_ygrad, indices, segment_ids, output_dim0, is_sqrtn)
